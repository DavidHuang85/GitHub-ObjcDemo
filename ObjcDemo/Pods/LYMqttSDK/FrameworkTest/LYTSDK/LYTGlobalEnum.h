...
typedef NS_ENUM(NSInteger,LYTMessageType) {
    ...
    LYTMessageTypeCustom            = 4000, // 自定义消息 字段 4000~9999均为自定义消息  由自己定义
    
    // 本地 辅助 消息 （例如:接收到讨论组添加成员消息时候 构造一条辅助消息保存到改讨论组回话当中）
    LYTMessageTypeLocalAssist       = 100000000, // 辅助类消息
    
    // 特殊消息
    LYTMessageTypeDistructRecepit   = 1011, // 点对点阅后即焚已读回执
    LYTMessageTypeReceipt           = 1000, // 回执消息
    LYTMessageTypeLineFeed          = 0000, // 换行标识
    LYTMessageTypePush              = 3993, // 推送消息

...
/**
 ...
 相关联的接口:didReceiveOnlineMessage: 和 didReceiveOfflineMessages：
 */
- (void)chatViewNeedRefreshDataWithSessionId:(NSString *)sessionId;

/**
 是否需要保存到本地数据库 不实现默认为YES
     这个方法会在 'didReceiveOnlineMessage:' 或者 'didReceiveOfflineMessages:' 之前调用
     在这里可以配置message属性  例如可以设置message的status 为已读 或者 添加一些attr字段
 
        -- 只要有一个代理实现了返回NO,就认为是NO    
        -- 当返回NO 时不会刷新列表

 @param message 消息是否需要保存到数据库
 @return 是否保存到本地数据库
 */
 ...
 * 发送新消息 / 重发消息   监听服务器回执
 ...
 @param message 消息模型
        -- 模型必须包含targetId、sessionId、sessionType、messageBody四个字段
